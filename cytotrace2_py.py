...
def cytotrace2(adata,               #input anndata object
               annotation_col = "", #column name of cell type annotation
               basis = None, # obsm embedding for plotting
               species = "mouse",
               batch_size = 20000,
    ...
    if not disable_plotting:
        # if embedding is provided, no data analysis using scanpy will be performed
        if basis is not None and len(basis) > 0:
            if basis in adata.obsm:
                print(f'cytotrace2: Using existing UMAP coordinates from adata.obsm["{basis}"]')
                adata_plot = adata.copy()
                # ensure that X_umap obsm is present
                if basis != 'X_umap':
                    adata_plot.obsm['X_umap'] = adata.obsm[basis]
                # cell type annotation for plotting
                if len(annotation_col) > 0:
                    adata_plot.obs['phenotype'] = adata.obs[annotation_col].copy()
                    adata_plot.obs['phenotype_txtwrap'] = [format_string(str(p)) for p in adata_plot.obs['phenotype']]
            else:
                raise ValueError(f"Specified umap key '{umap}' not found in adata.obsm. Available keys: {list(adata.obsm.keys())}")
        else:
            # using standard scanpy pipeline for data processing
            if not disable_verbose:
                print('cytotrace2: Computing UMAP embeddings from full expression')
            if len(annotation_col)>0:
                df_anno = adata.obs[[annotation_col]]
    ...
    # results of cytotrace analysis
    result_columns = ["CytoTRACE2_Score", "CytoTRACE2_Potency" , "CytoTRACE2_Relative", "preKNN_CytoTRACE2_Score", "preKNN_CytoTRACE2_Potency"]
    
    # adding cytrotrace result to adata.obs
    for col in result_columns:
        adata.obs[col] = predicted_df_final[col]

    # exporting cytrotrace results to local
    export_columns = result_columns.copy()
    if len(annotation_col) > 0:
        predicted_df_final[annotation_col] = adata.obs[annotation_col]
        export_columns.append(annotation_col)

    predicted_df_final[export_columns].to_csv(output_dir+'/cytotrace2_results.txt', sep='\t')

    if not disable_plotting:
        print('cytotrace2: Plotting outputs')
        if 'adata_plot' not in locals():
            adata_plot = adata
        
        # ploting using the cytrotrace results
        for col in result_columns:
            if col not in adata_plot.obs.columns:
                adata_plot.obs[col] = predicted_df_final.loc[adata_plot.obs_names, col]
